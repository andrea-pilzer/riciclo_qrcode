...
.run(function ($ionicPlatform, $rootScope, $ionicNavBarDelegate, $ionicHistory, $translate, $ionicPopup, $filter, $state, Profili, GeoLocate, $cordovaSplashscreen, $ionicLoading, $location) {
    ...
    $ionicPlatform.ready(function () {
        ...
        if(ionic.Platform.isAndroid()) {
            window.plugins.webintent.onNewIntent(
                qrCodeManager.read().then(
                    function(readQr) {
                        *var incomingUrl = readQr;*
                        console.log(readQr);
                        if(readQr !== "") {

                            console.log("app/raccolta/" + readQr.substring(7));
                            $location.path("app/raccolta/" + readQr.substring(7));
...
/* URL example: mypp://Carta e Cartone */
.controller('qrCodeController', function($scope, $location) {
	...
	$scope.reportAppLaunched = function(url) {
		...
		$scope.$apply( function() {
            ...
            if($scope.testo !== "") {
                            ...
                            console.log("app/raccolta/" + $scope.testo.substring(7));
                            $location.path("app/raccolta/" + $scope.testo.substring(7));
                            console.log($scope.testo);
                            }
		});
